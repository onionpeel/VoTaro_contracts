...
describe('GovernorAlpha', () => {
  ...
  xit('Retrieve all active proposals', async () => {
    ...
    const getActiveProposals = async () => {
      ...
      for(let i = 0; i <= proposalCount; i++) {
        let active = await governorAlpha.isProposalActive(ethers.BigNumber.from(i));
        if(active) {
          let proposal = await governorAlpha.proposals(ethers.BigNumber.from(i));
  ...
  xit('Retrieve inactive proposals', async () => {
    ...
    let currentlyInctiveProposals = await getInactiveProposals();
